...
def Start_F(message):
    while message.text == 'Привет' or message.text == 'Hello' or message.text == 'Hi' \
        ...
        bot.send_message(message.chat.id,
                         ...
                         "\nДавай определимся, что ты хотел бы найти по товару."
...
def Function_selection(message):
    if message.text == '🛒Товар' or message.text == 'Товар'.lower() or message.text == 'Выбор товара'.lower():
        bot.send_message(message.chat.id, '🏪Выберите магазин введя или нажав желаемый магазин. '
                                          'Если вы хотите ввернуться на шаг назад, введите или нажмите "🔙Назад". '
        ...
        bot.send_message(message.chat.id, 'До новых встреч, {0.first_name}! '
                                          'Для начала общения '
                                          'воспльзуйся клавиатурой-подсказкой.'.format(message.from_user,
...
def Shop_selection(message):
    ...
    if message.text == '🏪ДНС' or message.text == 'ДНС' or message.text == 'Днс':
        ...
        bot.send_message(message.chat.id, text_soobsh + ' Обязательно пишите без ключевых слов.'
                                                        ' Марку товара писать на английском языке. '
...
def Dns_product(message):
    ...
    bot.send_message(message.chat.id, 'Вот подборка товаров в магазине ДНС'
                                          f' :{corect_url}. Если вы желаете узнать наиболее меньшую'
                                          ' цену этого товара, напишите команду: Мин цена'
                                      '(или любую его интерпритацию на русском и английском).', reply_markup=mark_c)
...
def min_a_max(message):
    if message.text == "Мин цена" or message.text == "Минимальная цена" or message.text == "Min price" \
            or message.text == "Minimal prise" or message.text == "Мин. цена" or message.text == "Min. price":
        bot.send_message(message.chat.id, 'Напишите модель товара, который желаете купить.'
                                          ' Обязательно пишите без ключевых слов.'
                                          ' Марку товара писать на английском языке. '
                                          ' Пример ключевых слов: Монитор, Клавиатура.')
        bot.register_next_step_handler(message, E_Katalog)
    ...
    def parse():
        ...
        if html1.status_code == 200:
            ...
            bot.send_message(message.chat.id, "Минимальная цена найденого товара"
                                              ...
                                              " Желаете увидеть ссылку? Введите /yes или /no.", reply_markup=yes_n_mark)
...
def Video_print(message):
    ...
    bot.send_message(message.chat.id, f"Ссылка на видеообзор по запросу {slova_polz}: {corect_url}."
                                      " Желаете узнать отзывы по другому товару, введите: /otziv."
                                      " Желаете посмотреть товар, введите: /tovar"
                                      " Желаете посмотреть видеообзор на товар, введите: /videoobzor")
